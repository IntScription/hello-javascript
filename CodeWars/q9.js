...
const package_received_yesterday = (tz_from, tz_to, start, duration) => {
  const destination_send_time = start + (tz_to - tz_from);
  const arrival_time = destination_send_time + duration;

  // if < 0 : package arrived yesterday, if >= 0 then arrived today
  return arrival_time < 0 ? true : false;
};

console.log(package_received_yesterday(3, 0, 13, 1));
console.log(package_received_yesterday(12, -3, 5, 8));
